...
$$
\underbrace{R(\text{no propagation})}_{\text{baseline } B_0}
\;\ge\;
\underbrace{R(\text{scalar shrink, no graph})}_{B_1}
\;\ge\;
\underbrace{R(\text{learned bio-graph})}_{B_2},
$$
...
$$
\boxed{\,x' = T_\lambda\, x,\qquad T_\lambda \;=\; (1-\lambda)\,I \;+\; \lambda A,\qquad A=\tilde E\tilde E^\top\,}
\tag{1}
$$
...
* $\operatorname{rank}(A)\le r$ and eigenvalues $\mu_1\ge\dots\ge\mu_r\ge 0$,
  $\sum_i\mu_i=\operatorname{tr}(A)=G$.
...
**(A1) Observation model.** Each sample is
$$x = z + \varepsilon,\qquad \varepsilon\sim(0,\sigma^2 I_G),\ \sigma^2>0,\ \varepsilon\perp z,$$
where $z\in\mathbb{R}^G$ is the true biological signal and $\varepsilon$ is
isotropic measurement/technical noise.
...
$$
R(T)\;:=\;\mathbb{E}\bigl\lVert Tx - z\bigr\rVert_2^2 .
\tag{2}
$$
...
> and moreover
> $$
> R^\star(B_0)-R^\star(B_2)\;\ge\;\sum_{i=1}^{r}\frac{s_i^{2}}{s_i+\sigma^{2}}\;>\;0 .
> \tag{3}
> $$
...
> $$
> f_i^\star=\frac{s_i}{s_i+\sigma^2}\quad(i\le r),\qquad
> \text{realized by }\ v_i=u_i,\ \ (1-\lambda)+\lambda\mu_i=f_i^\star .
> \tag{4}
> $$
...
> $$
> R(T)=\sum_{k=1}^{G}\Bigl[(1-f_k)^2\,s_k+f_k^{2}\,\sigma^{2}\Bigr].
> \tag{5}
> $$
...
the second is $\sigma^2\sum_k f_k^2$. Summing gives (5). $\qquad\blacksquare$
...
> minimized at
> $$
> f_k^\star=\frac{s_k}{s_k+\sigma^2}\in[0,1),\qquad
> \text{residual}\ \;g(f_k^\star)=\frac{s_k\sigma^2}{s_k+\sigma^2}.
> \tag{6}
> $$
> Relative to the **no-filter** value on that direction, $g(1)=\sigma^2$, the
> optimum saves exactly
> $$
> \Delta_k \;:=\; g(1)-g(f_k^\star)\;=\;\sigma^2-\frac{s_k\sigma^2}{s_k+\sigma^2}\;=\;\frac{\sigma^{4}}{s_k+\sigma^{2}} .
> \tag{7}
> $$
...
$f_k^\star$:
$$g(f_k^\star)=\Big(\tfrac{\sigma^2}{s_k+\sigma^2}\Big)^2 s_k+
\Big(\tfrac{s_k}{s_k+\sigma^2}\Big)^2\sigma^2
=\frac{\sigma^2 s_k(\sigma^2+s_k)}{(s_k+\sigma^2)^2}=\frac{s_k\sigma^2}{s_k+\sigma^2}.$$
Hence the per-direction saving over $g(1)=\sigma^2$ is
$\Delta_k=\sigma^2-\frac{s_k\sigma^2}{s_k+\sigma^2}=\frac{\sigma^2(s_k+\sigma^2)-s_k\sigma^2}{s_k+\sigma^2}
=\frac{\sigma^4}{s_k+\sigma^2}$, which is (7). $\qquad\blacksquare$
...
optimum $f_k^\star\approx0$ — never increasing (5). $\qquad\blacksquare$
...
$$
R^\star(B_2)\le \sum_{i\le r}\frac{s_i\sigma^2}{s_i+\sigma^2}
\;+\;\sum_{k>r}\big[\lambda^2 s_k+(1-\lambda)^2\sigma^2\big].
$$
...
$B_0$). Therefore
$$
R^\star(B_0)-R^\star(B_2)\;\ge\;\sum_{i\le r}\Bigl(\sigma^2-\frac{s_i\sigma^2}{s_i+\sigma^2}\Bigr)
=\sum_{i\le r}\frac{\sigma^4}{s_i+\sigma^2}.
$$
Equivalently, since on the same directions the *signal-preservation* gain over
the maximally-shrunk baseline is $\frac{s_i^2}{s_i+\sigma^2}$ and
$\frac{\sigma^4}{s_i+\sigma^2}+\frac{s_i^2}{s_i+\sigma^2}
=\frac{s_i^2+\sigma^4}{s_i+\sigma^2}$, the guaranteed improvement over the
*better* of the two naive baselines is at least
$$
...
inequality above is strict. $\qquad\blacksquare$
...
holds only in the degenerate white-signal case $s_1=\dots=s_r$. $\qquad\blacksquare$
...
> $$
> \mathbb{E}\bigl\lVert \Phi(WTx)-\Phi(Wz)\bigr\rVert
> \;\le\; L_\Phi\,\lVert W\rVert_2\,\sqrt{R(T)} .
> \tag{8}
> $$
...
sets; monotonicity of $\min$ over set inclusion gives the claim. $\square$
...
$$
\begin{aligned}
&\textbf{No propagation } (B_0):\quad T=I,\qquad && R^\star(B_0)=G\sigma^2.\\[2pt]
&\textbf{Scalar shrink } (B_1):\quad T=(1-\lambda)I,\qquad && R^\star(B_1)=\min_{f}\sum_k\big[(1-f)^2s_k+f^2\sigma^2\big] \;\le\; R^\star(B_0).\\[2pt]
&\textbf{Learned bio-graph } (B_2):\quad T=(1-\lambda)I+\lambda \tilde E\tilde E^\top,\qquad
&& R^\star(B_2)=\sum_{i\le r}\tfrac{s_i\sigma^2}{s_i+\sigma^2}+(\text{tail}) \;\le\; R^\star(B_1),\\[2pt]
&\textbf{Guaranteed gap:} && R^\star(B_0)-R^\star(B_2)\;\ge\;\sum_{i\le r}\frac{s_i^2}{s_i+\sigma^2}\;>\;0.
\end{aligned}
$$
...
which the no-graph baseline provably cannot reach. $\blacksquare$
