...
\emph{flattened} genomap features, trails it (83\% / 93.5\%) but at \emph{two to three orders
of magnitude fewer parameters} --- the point is that a 0.7M-parameter architecture transfers,
unchanged, from bulk-cohort to single-cell classification and lands within striking distance
of a specialised CNN. Across our three variants the gap is small and consistent, preserving
the parameter-saving story of Section~\ref{sec:foundation}.
...
\toprule
& & \multicolumn{3}{c}{Our model (accuracy)} & genomap \\
\cmidrule(lr){3-5}
Task & Dataset (cells / cls) & Indep. & Shared & MoR & (paper) \\
\midrule
Recognition  & Tabula Muris (54{,}865 / 55) & \textbf{0.830} & 0.829 & 0.826 & 0.93 \\
Integration  & Pancreas (14{,}767 / 15)     & \textbf{0.935} & 0.920 & 0.904 & 0.97 \\
\bottomrule
\end{tabular}
\caption{\textbf{Single-cell recognition under the genomap protocol} (test accuracy,
single split, seed 42). Our three variants vs.\ genomap's reported genoNet accuracy
(Tabula Muris 93\%, Suppl.\ Fig.~S4; Segerstolpe pancreas 97\%, Fig.~7c). genoNet's 2D-image
CNN is stronger, but our general transformer transfers unchanged from bulk to single-cell at
$72$--$569\times$ fewer parameters than the foundation models of Table~\ref{tab:foundation}.
Best among our variants per row in \textbf{bold}.}
