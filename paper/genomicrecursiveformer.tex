...
As shown in Table~\ref{tab:singlecell}, on the pancreas integration our Independent
variant reaches 93.5\%, matching genomap's reported 93\% (which is 6\% / 21\% above
Cell-ID / SingleR), and the shared and MoR variants stay within $\sim$3 points at one
quarter the recurrent parameters.
...
\midrule
Recognition  & Tabula Muris (54{,}865 / 55) & \textbf{0.830} & 0.829 & 0.826 & Suppl.\,S4 \\
Integration  & Pancreas (14{,}767 / 15)     & \textbf{0.935} & 0.920 & 0.904 & 0.93 \\
\bottomrule
\end{tabular}
\caption{\textbf{Single-cell recognition under the genomap protocol} (test accuracy,
single split, seed 42). Our three variants vs.\ the genomap paper's reported genoNet
accuracy. On pancreas integration the Independent variant matches the paper's 93\%; the
shared and MoR variants reach it within $\sim$3 points at $\sim$4$\times$ fewer recurrent
parameters. Genomap's Tabula Muris accuracy is reported only in its Supplementary Fig.~S4.
Best per row (among our variants) in \textbf{bold}.}
