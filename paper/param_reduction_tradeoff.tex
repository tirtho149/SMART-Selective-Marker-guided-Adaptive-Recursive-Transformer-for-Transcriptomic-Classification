...
our three model variants under the genomap paper's protocol, with the genomap paper's
own reported genoNet accuracy for reference. On pancreas integration our Independent
variant matches the paper's headline 93\%. Best per row (among our variants) in
\textbf{bold}.}
...
\midrule
Cell recognition  & Tabula Muris (54{,}865 / 55) & \best{0.830} & 0.829 & 0.826 & Suppl.\ S4 \\
Data integration  & Pancreas (14{,}767 / 15)     & \best{0.935} & 0.920 & 0.904 & 0.93 \\
\bottomrule
\end{tabular}
\end{table}
...
The genomap paper reports 93\% on the pancreas integration (6\% / 21\% above Cell-ID /
SingleR; Segerstolpe subset 97\%); its Tabula Muris accuracy is given only in
Supplementary Fig.~S4. Our recursive model reaches the paper's pancreas accuracy at
$\sim$4$\times$ fewer recurrent parameters (shared / MoR) than the Independent stack
(see Table~\ref{tab:main}). Source: \texttt{results\_singlecell\_paper/*/}.
