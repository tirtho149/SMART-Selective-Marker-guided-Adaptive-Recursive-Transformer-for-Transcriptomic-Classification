...
\toprule
& & \multicolumn{3}{c}{\textbf{Our model (accuracy)}} & \textbf{genomap} \\
\cmidrule(lr){3-5}
\textbf{Task} & \textbf{Dataset (cells / cls)} &
\textbf{Independent} & \textbf{Shared} & \textbf{MoR} & \textbf{(paper)} \\
\midrule
Cell recognition  & Tabula Muris (54{,}865 / 55) & \best{0.830} & 0.829 & 0.826 & 0.93 \\
Data integration  & Pancreas (14{,}767 / 15)     & \best{0.935} & 0.920 & 0.904 & 0.97 \\
\bottomrule
\end{tabular}
\end{table}
...
\vspace{0.3em}
\noindent\footnotesize
\textit{Notes.} Accuracy on the paper's held-out test set (single split, seed 42).
genomap's genoNet reaches 93\% on Tabula Muris (Suppl.\ Fig.~S4; 6\% / 21\% above Cell-ID /
SingleR) and 97\% on the Segerstolpe pancreas subset (Fig.~7c). genoNet, a CNN purpose-built
for the 2D genomap image, is stronger; our value is that the \emph{same} 0.7M-parameter
recursive transformer used for the TCGA cohorts (Table~\ref{tab:main}) transfers to single-cell
unchanged. Source: \texttt{results\_singlecell\_paper/*/}.
