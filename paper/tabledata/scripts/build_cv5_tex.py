...
# CANONICAL bioMoR = ERA (E=embedding smoothing + R=graph-conv router + A=attention bias) at
# EVERY ladder cell -- single-cell (learned gene-graph attention) and multi-omics (Reactome
# pathway attention) are now apple-to-apple, no dataset discrimination (BIOLOGY_SC_VS_PATHWAY.md).
# Expert-choice carries attention via the per-step (k,k) sub-mask, so ALL K/routing cells are ERA.
def _eracfg(mode, K): return f"{mode}_k{K}"
def era_sc_l(ds, mode, K):
    return _read(f"results/cv5/biomor_era_ladder/{_eracfg(mode,K)}/{SCdir[ds]}/bio_era_cv.json")
def era_mo_l(coh, mode, K):
    return _read(f"results/cv5/biomor_era_ladder_mo/{_eracfg(mode,K)}/{coh}__*_cv.json")
...
def row_vals(kind, variant, K, mode):
    if kind=='biomor_eral':
        # CANONICAL bioMoR = ERA at (mode, K): read the era ladder directly. All 6 MO cohorts
        # (incl. tri-modal 3M) live in biomor_era_ladder_mo, so return sc/pn/pan complete here.
        sc=[era_sc_l(d,mode,K) for d in SC]
        pn=[era_mo_l(c,mode,K) for c in PN]
        pan=[era_mo_l(c,mode,K) for c in PANCAN]+[era_mo_l('pan_meta_pri_3modal',mode,K)]
        return sc, pn, pan
    if kind=='biomor_head':
...
SPECS=[  # (label,variant,mode,K,type,param,kind)
 ...
 # CANONICAL bioMoR = ERA (E+R+A) at EVERY ladder cell -- no ER/EA sub-combos, no separate
 # attention row. Expert-choice K2/3/4 and Token-choice K2/3/4 all carry attention bias.
 ("bioMoR (ERA)",None,'expert',2,"Expert","75K",'biomor_eral'),
 ("",None,'expert',3,"Expert","75K",'biomor_eral'),
 ("",None,'expert',4,"Expert","75K",'biomor_eral'),
 ("\\quad + Token ($N_R{=}2$)",None,'token',2,"Token","75K",'biomor_eral'),
 ("\\quad + Token ($N_R{=}3$)",None,'token',3,"Token","75K",'biomor_eral'),
 ("\\quad + Token choice",None,'token',4,"Token","75K",'biomor_eral'),
]
