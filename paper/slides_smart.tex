...
\begin{tikzpicture}[node distance=6.5mm]
  ...
  \node[stage, right=of mtok, draw=accentA, line width=1.2pt, fill=panelA, text width=22mm]
        (gint) {{\textcolor{accentA}{\faProjectDiagram}}\\[1pt]\textbf{Gene / Pathway Graph}\\[1pt]{\tiny\textcolor{subcap}{fixed co-expr.\,/\,Reactome \emph{or} \textbf{learned} $\pi$}}};
...
% ======================= SLIDE 2: results ==================================
\begin{frame}{Results: learned routing wins; efficiency holds; fixed priors don't}
\begin{columns}[T]
\column{0.5\textwidth}
\centering\textbf{\footnotesize Token reduction}\\[-1mm]
{\scriptsize macro-F1 vs \# marker tokens $M$ (13-dataset suite)}\\[1mm]
\resizebox{\linewidth}{!}{%
\begin{tabular}{lcccc}
\toprule
\rowcolor{nathead}
...
\midrule
Baron    & 33.7 & 53.9 & 62.6 & \cellcolor{natgood}66.9 \\
Lung     & 30.8 & 61.5 & 71.4 & \cellcolor{natgood}78.4 \\
T-cell   & 25.5 & 43.1 & 47.4 & \cellcolor{natgood}61.5 \\
Prostate & 74.3 & \cellcolor{natgood}78.0 & 76.7 & 77.3 \\
\bottomrule
\end{tabular}}
\vspace{2mm}

\centering\textbf{\footnotesize Vanilla vs Recursive vs MoR @ $K{=}4$}\\[1mm]
\resizebox{\linewidth}{!}{%
\begin{tabular}{lrrr}
\toprule
\rowcolor{nathead}
suite mean (13) & Vanilla & Recursive & MoR \\
\midrule
Params (unique) & 299{,}520 & \cellcolor{natgood}74{,}880 & \cellcolor{natgood}75{,}168 \\
FLOPs (rel.)    & 1.00$\times$ & 1.00$\times$ & \cellcolor{natgood}0.62$\times$ \\
Compute saved   & --- & --- & \cellcolor{natgood}38\% \\
macro-F1        & 51.4$\pm$16.8 & 52.3$\pm$17.1 & 50.4$\pm$17.1 \\
accuracy        & 66.2 & 67.1 & 66.6 \\
\bottomrule
\end{tabular}}\\[1mm]
...
\begin{tikzpicture}[node distance=1mm]
  ...
  \node[font=\scriptsize, text=accentB, align=center] at (2.6,1.55)
     {mean depth $2.75/4$\\[-1pt]\comp{$\sim$38\% fewer FLOPs}};
\end{tikzpicture}}
\vspace{1mm}

\begin{block}{\footnotesize Verdicts (multi-seed, 13 datasets)}
\scriptsize
\good{$+$} \good{Learned} routing graph: \good{$+12.0$ F1} single cell (66.5 vs 54.5), the decisive win --- beats fixed biology, random \emph{and} no prior.\\[2pt]
\comp{$+$} Adaptive depth: \comp{$\sim$38\% compute saved} at matched accuracy \& params.\\[2pt]
\bad{$-$} \emph{Fixed} biology prior: $=$ its random-graph control, \bad{collapses on some suites}; calibration \bad{no gain}.
\end{block}
\end{columns}
\end{frame}
...
\end{itemize}
\vspace{1mm}
{\footnotesize\good{Take-away:} token reduction is the biologically meaningful lever that \emph{works} --- and a \good{learned} routing graph (next slide) turns the biology-prior direction into a real accuracy gain.}
\end{columns}
\end{frame}
...
\end{block}
\vspace{1mm}
{\footnotesize\good{On data (10 seeds):} learned $-$ none $=\mathbf{+12.0}$ F1 on single
cell (66.5 vs 54.5; up to \good{$+19$} on Baron / T-cell) and \good{$+1.9$} on P-NET
(\good{$+5.7$} prostate). Beats co-expression, Reactome \emph{and} random --- the first
mechanism that does.}
\end{columns}
\end{frame}
