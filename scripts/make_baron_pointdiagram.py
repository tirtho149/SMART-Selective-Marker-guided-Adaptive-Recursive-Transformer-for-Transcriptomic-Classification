...
metric; the early-stopping/test metric); X axis = estimated cloud cost (USD, epoch-wise cumulative GPU-sec x rate). Reads the same
...
A100_USD_PER_HR = 1.80          # same A100 cloud rate as the training-cost curves (make_baron_epoch_figs)


def usd(sec):                   # epoch-wise cumulative GPU-seconds -> estimated cloud cost (USD)
    return sec / 3600.0 * A100_USD_PER_HR
...
def main():
    ...
    # label offset in POINTS (px), so it is independent of the tiny USD x-scale: (dx, dy, ha, va)
    LAB = {"Vanilla": (5, 6, "left", "bottom"), "Recursive": (5, 6, "left", "bottom"),
           "MoR": (6, -7, "left", "top"), "bioMoR": (5, 6, "left", "bottom")}
    ...
    for name in ORDER:
        ...
        if not agg:
            continue
        # cost (x) at the best-loss and best-accuracy checkpoints, in USD; y = model TEST accuracy
        xb = usd(min(agg, key=lambda e: e["loss_mean"])["sec_mean"])   # cost at best training loss
        xa = usd(max(agg, key=lambda e: e["val_f1_mean"])["sec_mean"]) # cost at best val-F1 (early stop)
        ...
        dx, dy, ha, va = LAB.get(name, (5, 6, "left", "bottom"))
        ax.annotate(f"{name} ({yt:.1f})", (max(xb, xa), yt), textcoords="offset points",
                    xytext=(dx, dy), ha=ha, va=va, fontsize=7.4, fontweight="bold", color=c, zorder=5)
    ...
    ax.set_xlabel(f"estimated cloud cost (USD, A100 @ ${A100_USD_PER_HR:.2f}/hr)", fontsize=8.5)
