# ============================================================================
# bioMoR -- full AAAI-style paper. One coherent story: bioMoR makes parameter
...
\pdfinfo{
/Title (bioMoR: A Selective Marker-guided Adaptive Recursive Transformer for Genomic Classification)
/Author (Koushik Howlader, Tirtho Roy, Md Tauhidul Islam, Wei Le)
}

\title{bioMoR: A Selective Marker-guided Adaptive Recursive Transformer\\
...
We present \textbf{bioMoR}, which (i) compresses thousands of genes into a small set
...
bioMoR rejects both. It keeps only a small set of \emph{interpretable tokens} and
...
adaptively-routed transformer. Because bioMoR is a one-shot \emph{set} classifier
...
(1) bioMoR, a recursive transformer that makes parameter efficiency architectural and
...
\section{The bioMoR Model}
\paragraph{Interpretable tokens.}
A gene-identity embedding plus a value projection embeds each input feature. bioMoR
...
\section{Conclusion}
bioMoR makes parameter efficiency architectural and computation adaptive for genomic
