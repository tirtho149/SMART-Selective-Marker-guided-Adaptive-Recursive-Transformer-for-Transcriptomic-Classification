...
architectural choices are statistically within noise. The whole
...
\node[stage, right=of shared] (mor) {{\large\textcolor{accentB}{\faFilter}}\\[2pt]\textbf{MoR Depth Router}\\[1pt]{\scriptsize\textcolor{subcap}{capacity funnel, depth $d_m$}}};
...
\node[stage, right=of clf] (coh) {{\large\textcolor{accentB}{\faSitemap}}\\[2pt]\textbf{4 Cohorts}\\[1pt]{\tiny\textcolor{subcap}{BRCA / HNSC\\ LUNG / THCA}}};
...
candidate drivers). Tokens are mean-pooled and classified into the four TCGA cohorts.}
\label{fig:overview}
\end{figure*}
...
work. We report these so the trade-offs are not overstated.
...
parameters and recovers biologically meaningful genes. Evaluated on the full
transcriptome across five clinical phenotype tasks it achieves the best mean
macro-F1 against ten strong nonlinear baselines while remaining far smaller, and it
transfers unchanged to single-cell data; our multi-seed study also delineates,
honestly, where the marker-learning advantage does and does not hold. The complete
pipeline, including all experiments, baselines, and this paper, regenerates from a
single command.
