"""BRAINSTORM PROTOTYPE (not wired into the paper).
...
for s in B.SPECS:
    ...
    sc, pn, pan = B.row_vals(kind, variant, K)
    v = (sc + pn + pan)[BARON]
...
fig.tight_layout()
import os as _os
_figs = _os.path.join(_os.path.dirname(__file__), "paper", "figs")
_os.makedirs(_figs, exist_ok=True)
fig.savefig(_os.path.join(_figs, "pareto_efficiency.pdf"), bbox_inches="tight", dpi=600)
fig.savefig("pareto_prototype.pdf", bbox_inches="tight", dpi=600)
